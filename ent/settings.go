...
type Settings struct {
	...
	// AllowAllUser holds the value of the "allow_all_user" field.
	AllowAllUser bool `json:"allow_all_user,omitempty"`
...
func (*Settings) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case settings.FieldID, settings.FieldChatID:
	...
	for i := range columns {
		...
		case settings.FieldChatID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				s.ChatID = value.Int64
			}
		case settings.FieldAllowAllUser:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field allow_all_user", values[i])
			} else if value.Valid {
				s.AllowAllUser = value.Bool
			}
		default:
...
func (s *Settings) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", s.ChatID))
	builder.WriteString(", ")
	builder.WriteString("allow_all_user=")
	builder.WriteString(fmt.Sprintf("%v", s.AllowAllUser))
	builder.WriteByte(')')
